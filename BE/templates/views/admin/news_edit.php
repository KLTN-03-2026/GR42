            ...
            <div class="col-12 col-xl-8">
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-white border-bottom py-3">
                        <h6 class="mb-0 fw-bold text-primary"><i class="fa-solid fa-pen-to-square me-2"></i>Nội dung bài
                            viết</h6>
                    </div>
                    <div class="card-body p-4">
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted">Tiêu đề (Title) <span
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="description" rows="3"
                                placeholder="Tóm tắt ngắn gọn bài báo..."><?= htmlspecialchars($article['description'] ?? '') ?></textarea>
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="editor" class="form-control" name="content" rows="12"
                                placeholder="Nhập nội dung đầy đủ..."><?= htmlspecialchars($article['content'] ?? '') ?></textarea>
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4 text-center">
                            <label class="form-label fw-bold small text-muted d-block text-start">Ảnh đại diện
                                (Thumbnail URL)</label>
                            <div class="bg-light rounded-3 overflow-hidden mb-3 d-flex align-items-center justify-content-center border"
                                style="height: 180px;">
                                <?php if(!empty($article['thumbnail'])): ?>
                                <img src="<?= htmlspecialchars($article['thumbnail']) ?>" alt="Thumbnail"
                                    class="img-fluid" style="max-height: 100%; width: 100%; object-fit: cover;">
                                <?php else: ?>
                                <span class="text-muted"><i class="fa-regular fa-image fs-1 mb-2 d-block"></i>Chưa có
                                    ảnh</span>
                                <?php endif; ?>
                            </div>
                            <input type="text" class="form-control form-control-sm" name="thumbnail"
                                value="<?= htmlspecialchars($article['thumbnail'] ?? '') ?>" placeholder="https://...">
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" name="view"
                                value="<?= (int)($article['view'] ?? 0) ?>">
                        </div>
                        <?php endif; ?>

                        <div class="border-top pt-3 mt-4">
                            <small class="text-muted d-block mb-1">Ngày tạo:
                                <strong><?= isset($article['created_at']) ? date('d/m/Y H:i', strtotime($article['created_at'])) : 'Không rõ' ?></strong></small>
                            <small class="text-muted d-block">Lần sửa cuối:
                                <strong><?= isset($article['updated_at']) ? date('d/m/Y H:i', strtotime($article['updated_at'])) : 'Vừa xong' ?></strong></small>
                        </div>
                    </div>
                </div>
            </div>
