...
const AdminNewsList = () => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (res.data.status === 'success') {
                ...
                showToast('Xóa bài báo thành công!', 'success');
    ...
    const handleCrawl = async () => {
        ...
        try {
            ...
            if (res.data.status === 'success') {
                ...
                showToast('Lỗi: ' + (res.data.message || 'Thất bại khi thu thập dữ liệu'), 'error');
            }
        } catch (error) {
            console.error('Error crawling news:', error);
            showToast('Lỗi kết nối máy chủ khi thu thập dữ liệu!', 'error');
        } finally {
            setCrawling(false);
        }
    };
    ...
    useEffect(() => {
        fetchNews(1, initialSearch, filterCategory);
    }, [initialSearch]);
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 text-center">
                                <p className="text-[10px] font-black text-emerald-600 tracking-widest mb-1">Bài viết mới</p>
                                <p className="text-2xl font-black text-emerald-700">{crawlResults?.new || 0}</p>
                            </div>
                            <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 text-center">
                                <p className="text-[10px] font-black text-slate-400 tracking-widest mb-1">Tổng dữ liệu</p>
