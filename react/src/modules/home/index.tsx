...
import { Search, Heart, MessageCircle, Share2, Loader2, ChevronLeft, ChevronRight, Clock } from 'lucide-react';
...
const Home = () => {
  ...
  const fetchUserInteractions = useCallback(async () => {
    ...
    try {
      ...
      if (favRes.data.status === 'success') {
        setFavorites(favRes.data.data.slice(0, 5));
      }
  ...
  useEffect(() => {
    ...
    setPage(1);
    window.scrollTo(0, 0);
  }, [selectedCategory, currentKeyword, fetchNews, fetchUserInteractions]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pb-20">
      <div className="flex items-center gap-8 py-8 overflow-x-auto no-scrollbar mb-8 border-b border-slate-50 scroll-smooth">
        {categories.map((cat) => (
          <button
            key={cat.id}
                ...
                <div className="sticky top-24 space-y-12">
                  {favorites.length > 0 && (
                    <div className="space-y-6">
                      <div className="flex items-center gap-4">
                        <div className="w-1.5 h-6 bg-red-500 rounded-full shadow-sm shadow-red-200"></div>
                        <h3 className="font-black text-slate-900 tracking-widest text-sm flex items-center gap-2">
                      ...
                      <div className="space-y-6">
                        {favorites.map((item) => (
                          <div 
                            ...
                            onClick={() => navigate(`/article/${encodeId(item.id)}`)}
                          >
                            <div className="w-20 h-14 rounded-xl overflow-hidden shrink-0 border border-slate-100 bg-slate-50">
                              <img src={item.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform" alt="" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="text-[13px] font-bold text-slate-800 line-clamp-2 group-hover:text-blue-600 transition-colors leading-relaxed">
                                {item.title}
                              </h4>
                            </div>
                          </div>
                  ...
                  <div className="space-y-8">
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <h3 className="font-black text-slate-900 tracking-widest text-sm">Tin nhanh</h3>
                    </div>
                    <div className="space-y-8">
                        ...
                        >
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-[9px] font-bold text-slate-300 tracking-widest">{item.pubDate}</span>
                            </div>
                            <h4 className="font-bold text-slate-800 line-clamp-3 group-hover:text-blue-600 transition-colors leading-relaxed">
                              {item.title}
                            </h4>
                            <div className="flex items-center gap-4 mt-2">
                              <span className="text-[9px] font-black text-slate-400 tracking-tighter">{item.source}</span>
