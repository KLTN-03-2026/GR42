  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (searchTerm.trim().length >= 2) {
        ...
        if (location.pathname === '/' || location.search.includes('keyword')) {
          navigate(`/?keyword=${encodeURIComponent(searchTerm.trim())}`, { replace: true });
        }
      } else {
        setSuggestions([]);
        setShowSuggestions(false);
        if (searchTerm.trim() === '' && location.search.includes('keyword')) {
